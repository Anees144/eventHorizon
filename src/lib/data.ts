

import type { Event, User, PromoCode } from './types';
import { PlaceHolderImages } from './placeholder-images';

const getImage = (hint: string) => {
    return PlaceHolderImages.find(img => img.imageHint.includes(hint)) || PlaceHolderImages[0];
}

export const user: User = {
    id: 'user-1',
    name: 'Alex Doe',
    email: 'alex.doe@example.com',
    avatarUrl: getImage('user avatar').imageUrl,
    avatarHint: getImage('user avatar').imageHint,
    interests: ['Music', 'Technology', 'Art']
};

export const initialEvents: Event[] = [
    {
        id: '1',
        title: 'Synthwave Night',
        description: 'Dive into the retro-futuristic sounds of synthwave with top DJs.',
        richDescription: '<p>Join us for an electrifying night of retro-futuristic sounds. <strong>Synthwave Night</strong> brings together the best DJs in the genre for an unforgettable audio-visual experience. Expect neon lights, classic arcade games, and a dance floor that won\'t quit.</p><p>Dress in your best 80s attire and get ready to travel back to the future.</p>',
        imageUrl: getImage('music concert').imageUrl,
        imageHint: getImage('music concert').imageHint,
        videoUrl: 'https://www.youtube.com/watch?v=4x1wi-4Kq9E',
        category: 'Music',
        date: '2024-08-15T20:00:00.000Z',
        location: 'Cyber City, Neo-Tokyo',
        latitude: 35.6895,
        longitude: 139.6917,
        organizer: 'RetroWave Productions',
        organizerId: 'user-1',
        tags: ['#synthwave', '#80s', '#retro', '#music'],
        visibility: 'public',
        ticketTiers: [
            { id: 't1-1', name: 'General Admission', price: 25, type: 'paid' },
            { id: 't1-2', name: 'VIP Access', price: 75, type: 'paid' },
        ],
        promoCodes: [
            { code: 'SYNTH10', discountPercentage: 10 },
            { code: 'WAVE20', discountPercentage: 20 },
        ]
    },
    {
        id: '2',
        title: 'AI Innovators Summit 2024',
        description: 'Explore the future of Artificial Intelligence with industry leaders.',
        richDescription: '<p>The <strong>AI Innovators Summit</strong> is a premier event for developers, researchers, and entrepreneurs in the AI space. This year, we focus on the impact of generative AI on creativity and business.</p><ul><li>Keynotes from industry pioneers</li><li>Hands-on workshops</li><li>Networking sessions</li></ul>',
        imageUrl: getImage('tech conference').imageUrl,
        imageHint: getImage('tech conference').imageHint,
        videoUrl: undefined,
        category: 'Technology',
        date: '2024-09-10T09:00:00.000Z',
        location: 'Innovation Hall, Silicon Valley',
        latitude: 37.4013,
        longitude: -122.122,
        organizer: 'TechForward',
        organizerId: 'user-2',
        tags: ['#AI', '#MachineLearning', '#Tech', '#Innovation'],
        visibility: 'public',
        ticketTiers: [
            { id: 't2-1', name: 'Student Pass', price: 100, type: 'paid' },
            { id: 't2-2', name: 'Full Access', price: 400, type: 'paid' },
            { id: 't2-3', name: 'Corporate Pass', price: 1200, type: 'paid' },
        ],
        promoCodes: []
    },
    {
        id: '3',
        title: 'Colors of Tomorrow',
        description: 'An immersive art exhibition showcasing digital and traditional media.',
        richDescription: '<p><strong>Colors of Tomorrow</strong> is an art exhibition that blurs the lines between the physical and digital worlds. Featuring works from over 30 international artists, this gallery explores themes of nature, identity, and the future.</p><p>Interactive installations and VR experiences will be available.</p>',
        imageUrl: getImage('art exhibition').imageUrl,
        imageHint: getImage('art exhibition').imageHint,
        videoUrl: 'https://vimeo.com/183786263',
        category: 'Art',
        date: '2024-07-20T10:00:00.000Z',
        location: 'The Modern Gallery, Paris',
        latitude: 48.8566,
        longitude: 2.3522,
        organizer: 'ArtConnect',
        organizerId: 'user-3',
        tags: ['#art', '#exhibition', '#digitalart', '#modernart'],
        visibility: 'public',
        ticketTiers: [
            { id: 't3-1', name: 'Single Entry', price: 15, type: 'paid' },
        ],
        promoCodes: []
    },
    {
        id: '4',
        title: 'Global Bites Festival',
        description: 'Taste the world in one place. A festival for all food lovers.',
        richDescription: '<p>Embark on a culinary journey at the <strong>Global Bites Festival</strong>. With over 50 food trucks and stalls, you can savor dishes from every corner of the globe. Live music and family-friendly activities make this a perfect day out.</p>',
        imageUrl: getImage('food festival').imageUrl,
        imageHint: getImage('food festival').imageHint,
        videoUrl: undefined,
        category: 'Food',
        date: '2024-08-03T11:00:00.000Z',
        location: 'Waterfront Park, New York',
        latitude: 40.7128,
        longitude: -74.0060,
        organizer: 'TasteMakers Inc.',
        organizerId: 'user-1',
        tags: ['#food', '#festival', '#streetfood', '#familyfriendly'],
        visibility: 'public',
        ticketTiers: [
            { id: 't4-1', name: 'Entry', price: 10, type: 'paid' },
            { id: 't4-2', name: 'Tasting Passport', price: 50, type: 'paid' },
        ],
        promoCodes: []
    },
    {
        id: '5',
        title: 'The Startup Pitch Battle',
        description: 'Watch the next generation of entrepreneurs compete for funding.',
        richDescription: '<p>The annual <strong>Startup Pitch Battle</strong> is back! Witness the most innovative startups pitch their ideas to a panel of venture capitalists. The winner takes home a grand prize to kickstart their business. A must-attend for aspiring entrepreneurs and investors.</p>',
        imageUrl: getImage('business seminar').imageUrl,
        imageHint: getImage('business seminar').imageHint,
        videoUrl: undefined,
        category: 'Business',
        date: '2024-10-05T18:00:00.000Z',
        location: 'The Enterprise Hub, London',
        latitude: 51.5074,
        longitude: -0.1278,
        organizer: 'Innovate UK',
        organizerId: 'user-2',
        tags: ['#startup', '#pitch', '#vc', '#entrepreneur'],
        visibility: 'public',
        ticketTiers: [
            { id: 't5-1', name: 'Spectator', price: 20, type: 'paid' },
            { id: 't5-2', name: 'Investor Access', price: 150, type: 'paid' },
        ],
        promoCodes: []
    },
    {
        id: '6',
        title: 'Mindfulness & Yoga Retreat',
        description: 'A weekend of peace, meditation, and rejuvenation.',
        richDescription: '<p>Escape the hustle and bustle with our <strong>Mindfulness & Yoga Retreat</strong>. Set in the serene countryside, this weekend retreat offers guided meditation, vinyasa flow yoga, and workshops on mindful living. All levels welcome.</p>',
        imageUrl: getImage('yoga workshop').imageUrl,
        imageHint: getImage('yoga workshop').imageHint,
        videoUrl: undefined,
        category: 'Wellness',
        date: '2024-09-27T16:00:00.000Z',
        location: 'Serenity Gardens, Bali',
        latitude: -8.3405,
        longitude: 115.0920,
        organizer: 'ZenLife',
        organizerId: 'user-3',
        tags: ['#yoga', '#meditation', '#wellness', '#retreat'],
        visibility: 'public',
        ticketTiers: [
            { id: 't6-1', name: 'All-Inclusive Weekend', price: 500, type: 'paid' },
        ],
        promoCodes: []
    },
    {
        id: '7',
        title: 'Indie Rock Fest',
        description: 'Discover your new favorite indie bands at this day-long festival.',
        richDescription: '<p><strong>Indie Rock Fest</strong> is a celebration of independent music. Featuring three stages and over 20 bands, it\'s a full day of great tunes, food trucks, and good vibes.</p>',
        imageUrl: getImage('music concert').imageUrl,
        imageHint: 'music concert',
        videoUrl: undefined,
        category: 'Music',
        date: '2024-09-21T14:00:00.000Z',
        location: 'Greenfield Park, Austin',
        latitude: 30.2672,
        longitude: -97.7431,
        organizer: 'SoundWave Events',
        organizerId: 'user-1',
        tags: ['#indierock', '#musicfestival', '#livemusic'],
        visibility: 'public',
        ticketTiers: [
            { id: 't7-1', name: 'Early Bird', price: 40, type: 'paid' },
            { id: 't7-2', name: 'General Admission', price: 60, type: 'paid' },
        ],
        promoCodes: []
    },
    {
        id: '8',
        title: 'Future of Web3',
        description: 'A deep dive into the decentralized web, blockchain, and NFTs.',
        richDescription: '<p>Join the brightest minds in decentralized technology at the <strong>Future of Web3</strong> conference. We will cover everything from DeFi to DAOs and the new creator economy.</p>',
        imageUrl: getImage('tech conference').imageUrl,
        imageHint: 'tech conference',
        videoUrl: undefined,
        category: 'Technology',
        date: '2024-11-12T09:00:00.000Z',
        location: 'Virtual Event',
        latitude: 34.0522,
        longitude: -118.2437,
        organizer: 'CryptoCon',
        organizerId: 'user-2',
        tags: ['#web3', '#blockchain', '#crypto', '#nft'],
        visibility: 'public',
        ticketTiers: [
            { id: 't8-1', name: 'Virtual Pass', price: 50, type: 'paid' },
            { id: 't8-2', name: 'VIP Virtual Pass', price: 150, type: 'paid' },
        ],
        promoCodes: []
    },
    {
        id: '9',
        title: 'Night at the Theater: Hamlet',
        description: 'A modern interpretation of Shakespeare\'s classic tragedy.',
        richDescription: '<p>Experience <strong>Hamlet</strong> like never before. This production brings Shakespeare\'s timeless story of revenge, madness, and moral corruption into the 21st century with a powerful and minimalist aesthetic.</p>',
        imageUrl:getImage('theater show').imageUrl,
        imageHint: getImage('theater show').imageHint,
        videoUrl: undefined,
        category: 'Art',
        date: '2024-08-22T19:30:00.000Z',
        location: 'The Grand Stage, New York',
        latitude: 40.758,
        longitude: -73.9855,
        organizer: 'Modern Bard Company',
        organizerId: 'user-3',
        tags: ['#theater', '#shakespeare', '#play', '#drama'],
        visibility: 'public',
        ticketTiers: [
            { id: 't9-1', name: 'Balcony', price: 45, type: 'paid' },
            { id: 't9-2', name: 'Orchestra', price: 85, type: 'paid' },
        ],
        promoCodes: []
    },
    {
        id: '10',
        title: 'Championship Finals',
        description: 'Witness the climax of the season in this epic basketball showdown.',
        richDescription: '<p>The <strong>Championship Finals</strong> are here! The top two teams battle it out for glory. Expect a high-energy game with dazzling plays and a roaring crowd. Don\'t miss the action!</p>',
        imageUrl: getImage('sports game').imageUrl,
        imageHint: getImage('sports game').imageHint,
        videoUrl: 'https://www.youtube.com/watch?v=bf_4h-1zYwI',
        category: 'Sports',
        date: '2024-09-14T18:00:00.000Z',
        location: 'The Arena, Los Angeles',
        latitude: 34.043,
        longitude: -118.2673,
        organizer: 'National Basketball League',
        organizerId: 'user-4',
        tags: ['#basketball', '#finals', '#sports', '#nbl'],
        visibility: 'public',
        ticketTiers: [
            { id: 't10-1', name: 'Upper Bowl', price: 80, type: 'paid' },
            { id: 't10-2', name: 'Lower Bowl', price: 250, type: 'paid' },
            { id: 't10-3', name: 'Courtside', price: 1500, type: 'paid' },
        ],
        promoCodes: []
    },
    {
        id: '11',
        title: 'Park Cleanup & Picnic',
        description: 'Join us to beautify our local park and enjoy a community picnic afterwards.',
        richDescription: '<p>Let\'s make a difference together! At the <strong>Park Cleanup & Picnic</strong>, we\'ll spend the morning cleaning up our beloved community park. Afterwards, we\'ll celebrate our hard work with a free picnic for all volunteers.</p>',
        imageUrl: getImage('community meetup').imageUrl,
        imageHint: getImage('community meetup').imageHint,
        videoUrl: undefined,
        category: 'Community',
        date: '2024-08-17T09:00:00.000Z',
        location: 'Central Park, Springfield',
        latitude: 39.7817,
        longitude: -89.6501,
        organizer: 'Springfield Action Committee',
        organizerId: 'user-1',
        tags: ['#community', '#volunteer', '#cleanup', '#environment'],
        visibility: 'public',
        ticketTiers: [
            { id: 't11-1', name: 'Volunteer (Free)', price: 0, type: 'reservation' },
            { id: 't11-2', name: 'Support with a Donation', price: 0, type: 'donation' },
        ],
        promoCodes: []
    },
    {
        id: '12',
        title: 'E-Sports Grand Finals',
        description: 'The world\'s best gamers compete for the championship title.',
        richDescription: '<p>Feel the hype at the <strong>E-Sports Grand Finals</strong>! Watch pro players display incredible skill and strategy in the most popular competitive games. Live commentary, merchandise, and more.</p>',
        imageUrl: getImage('tech conference').imageUrl,
        imageHint: 'tech conference',
        videoUrl: undefined,
        category: 'Technology',
        date: '2024-09-28T12:00:00Z',
        location: 'Seoul e-Stadium, South Korea',
        latitude: 37.505,
        longitude: 127.075,
        organizer: 'Global Gaming League',
        organizerId: 'user-2',
        tags: ['#esports', '#gaming', '#tournament', '#finals'],
        visibility: 'public',
        ticketTiers: [
            { id: 't12-1', name: 'Floor Seats', price: 50, type: 'paid' },
            { id: 't12-2', name: 'Stadium Seating', price: 30, type: 'paid' }
        ],
        promoCodes: []
    },
    {
        id: '13',
        title: 'Jazz Under the Stars',
        description: 'An open-air jazz concert featuring local and international artists.',
        richDescription: '<p>Relax to the smooth sounds of <strong>Jazz Under the Stars</strong>. This magical evening features a lineup of talented jazz musicians in a beautiful outdoor amphitheater. Bring a blanket and enjoy the show.</p>',
        imageUrl: getImage('music concert').imageUrl,
        imageHint: 'music concert',
        videoUrl: undefined,
        category: 'Music',
        date: '2024-08-30T19:00:00Z',
        location: 'Moonlight Amphitheater, Chicago',
        latitude: 41.8781,
        longitude: -87.6298,
        organizer: 'Chicago Jazz Society',
        organizerId: 'user-1',
        tags: ['#jazz', '#livemusic', '#outdoor', '#concert'],
        visibility: 'public',
        ticketTiers: [
            { id: 't13-1', name: 'Lawn Seating', price: 25, type: 'paid' },
            { id: 't13-2', name: 'Reserved Chair', price: 45, type: 'paid' }
        ],
        promoCodes: []
    },
    {
        id: '14',
        title: 'Street Art Tour',
        description: 'A walking tour of the city\'s most vibrant street art and murals.',
        richDescription: '<p>Discover the hidden artistic gems of the city on our <strong>Street Art Tour</strong>. A local artist will guide you through alleys and streets, explaining the stories and techniques behind the incredible murals.</p>',
        imageUrl: getImage('art exhibition').imageUrl,
        imageHint: 'art exhibition',
        videoUrl: undefined,
        category: 'Art',
        date: '2024-09-07T14:00:00Z',
        location: 'Shoreditch, London',
        latitude: 51.5229,
        longitude: -0.0754,
        organizer: 'Urban Canvas Tours',
        organizerId: 'user-3',
        tags: ['#streetart', '#graffiti', '#tour', '#urbanart'],
        visibility: 'public',
        ticketTiers: [
            { id: 't14-1', name: 'Adult', price: 20, type: 'paid' },
            { id: 't14-2', name: 'Student', price: 15, type: 'paid' }
        ],
        promoCodes: []
    },
    {
        id: '15',
        title: 'Gourmet Chocolate Making',
        description: 'Learn the art of chocolate making from a master chocolatier.',
        richDescription: '<p>Indulge your sweet tooth and learn a new skill at our <strong>Gourmet Chocolate Making</strong> workshop. You\'ll learn tempering, molding, and flavoring techniques to create your own delicious confections to take home.</p>',
        imageUrl: getImage('food festival').imageUrl,
        imageHint: 'food festival',
        videoUrl: undefined,
        category: 'Food',
        date: '2024-10-12T13:00:00Z',
        location: 'The Cocoa Bean, Brussels',
        latitude: 50.8503,
        longitude: 4.3517,
        organizer: 'Belgian Chocolate School',
        organizerId: 'user-1',
        tags: ['#chocolate', '#workshop', '#dessert', '#culinary'],
        visibility: 'public',
        ticketTiers: [
            { id: 't15-1', name: 'Workshop Fee', price: 80, type: 'paid' }
        ],
        promoCodes: []
    },
    {
        id: '16',
        title: 'Women in Leadership Conference',
        description: 'An empowering conference for women in the business world.',
        richDescription: '<p>The <strong>Women in Leadership Conference</strong> brings together leaders, entrepreneurs, and executives to share their stories, insights, and strategies for success. Network with peers and get inspired to take your career to the next level.</p>',
        imageUrl: getImage('business seminar').imageUrl,
        imageHint: 'business seminar',
        videoUrl: undefined,
        category: 'Business',
        date: '2024-11-08T09:00:00Z',
        location: 'Metropolitan Conference Center, NYC',
        latitude: 40.7549,
        longitude: -73.984,
        organizer: 'LeadHers Network',
        organizerId: 'user-2',
        tags: ['#womeninbusiness', '#leadership', '#conference', '#networking'],
        visibility: 'public',
        ticketTiers: [
            { id: 't16-1', name: 'Early Bird', price: 250, type: 'paid' },
            { id: 't16-2', name: 'General Admission', price: 350, type: 'paid' }
        ],
        promoCodes: []
    },
    {
        id: '17',
        title: 'Digital Detox Weekend',
        description: 'Disconnect to reconnect. A weekend getaway without screens.',
        richDescription: '<p>Join our <strong>Digital Detox Weekend</strong> to unplug from technology and reconnect with yourself, others, and nature. Activities include hiking, campfires, journaling, and group discussions. All phones and devices will be securely stored for the duration.</p>',
        imageUrl: getImage('yoga workshop').imageUrl,
        imageHint: 'yoga workshop',
        videoUrl: undefined,
        category: 'Wellness',
        date: '2024-08-23T18:00:00Z',
        location: 'Redwood National Park, California',
        latitude: 41.2132,
        longitude: -124.0046,
        organizer: 'The Unplugged Movement',
        organizerId: 'user-3',
        tags: ['#digitaldetox', '#mindfulness', '#nature', '#retreat'],
        visibility: 'public',
        ticketTiers: [
            { id: 't17-1', name: 'Shared Cabin', price: 300, type: 'paid' },
            { id: 't17-2', name: 'Private Cabin', price: 450, type: 'paid' }
        ],
        promoCodes: []
    },
    {
        id: '18',
        title: 'Beach Volleyball Tournament',
        description: 'Sun, sand, and spikes! A friendly beach volleyball competition.',
        richDescription: '<p>Get your team together for our annual <strong>Beach Volleyball Tournament</strong>. All skill levels are welcome, from beginners to seasoned players. Prizes for the winning team and a beach party to follow.</p>',
        imageUrl: getImage('sports game').imageUrl,
        imageHint: 'sports game',
        videoUrl: undefined,
        category: 'Sports',
        date: '2024-07-27T10:00:00Z',
        location: 'Santa Monica Beach, CA',
        latitude: 34.0194,
        longitude: -118.4912,
        organizer: 'SoCal Sports League',
        organizerId: 'user-4',
        tags: ['#volleyball', '#beach', '#tournament', '#sports'],
        visibility: 'public',
        ticketTiers: [
            { id: 't18-1', name: 'Team Registration (4 players)', price: 100, type: 'paid' }
        ],
        promoCodes: []
    },
    {
        id: '19',
        title: 'Neighborhood Watch Meeting',
        description: 'Discuss community safety and initiatives with your neighbors.',
        richDescription: '<p>Join your neighbors for the monthly <strong>Neighborhood Watch Meeting</strong>. We will discuss recent activity, crime prevention tips, and upcoming community events. Your participation is key to a safer neighborhood for everyone.</p>',
        imageUrl: getImage('community meetup').imageUrl,
        imageHint: 'community meetup',
        videoUrl: undefined,
        category: 'Community',
        date: '2024-08-05T19:00:00Z',
        location: 'Community Hall, Maplewood',
        latitude: 40.7303,
        longitude: -74.2757,
        organizer: 'Maplewood Safety Alliance',
        organizerId: 'user-1',
        tags: ['#community', '#safety', '#neighborhoodwatch'],
        visibility: 'public',
        ticketTiers: [
            { id: 't19-1', name: 'Attendance', price: 0, type: 'reservation' }
        ],
        promoCodes: []
    },
    {
        id: '20',
        title: 'Laugh Riot Comedy Night',
        description: 'Get ready for a night of non-stop laughter with the city\'s best comedians.',
        richDescription: '<p><strong>Laugh Riot Comedy Night</strong> is your weekly dose of humor. Featuring a lineup of up-and-coming talent and seasoned pros, this is the perfect way to unwind and have a good time.</p>',
        imageUrl: getImage('comedy night').imageUrl,
        imageHint: 'comedy night',
        videoUrl: undefined,
        category: 'Art',
        date: '2024-10-18T20:00:00.000Z',
        location: 'The Funny Bone, Chicago',
        latitude: 41.8781,
        longitude: -87.6298,
        organizer: 'Comedy Central',
        organizerId: 'user-4',
        tags: ['#comedy', '#standup', '#humor', '#nightlife'],
        visibility: 'public',
        ticketTiers: [
            { id: 't20-1', name: 'General Admission', price: 20, type: 'paid' }
        ],
        promoCodes: []
    },
    {
        id: '21',
        title: 'An Evening with the Author',
        description: 'Join a bestselling author for a reading and Q&A session for their new novel.',
        richDescription: '<p>Don\'t miss <strong>An Evening with the Author</strong>, a special event featuring a reading from a new critically acclaimed novel, followed by an intimate Q&A session and book signing. A wonderful opportunity for book lovers to connect with a favorite writer.</p>',
        imageUrl: getImage('book reading').imageUrl,
        imageHint: 'book reading',
        videoUrl: undefined,
        category: 'Art',
        date: '2024-11-01T19:00:00.000Z',
        location: 'City Library, San Francisco',
        latitude: 37.7749,
        longitude: -122.4194,
        organizer: 'Readers Hub',
        organizerId: 'user-3',
        tags: ['#books', '#author', '#reading', '#literature'],
        visibility: 'public',
        ticketTiers: [
            { id: 't21-1', name: 'Free Entry', price: 0, type: 'reservation' }
        ],
        promoCodes: []
    },
    {
        id: '22',
        title: 'Run for a Cause 5K',
        description: 'Lace up your running shoes for a charity 5K run to support local shelters.',
        richDescription: '<p>The annual <strong>Run for a Cause 5K</strong> is a fantastic way to get active while supporting a great cause. All proceeds from the race will go directly to supporting local homeless shelters. Runners and walkers of all ages and abilities are welcome.</p>',
        imageUrl: getImage('charity run').imageUrl,
        imageHint: 'charity run',
        videoUrl: undefined,
        category: 'Community',
        date: '2024-09-22T09:00:00.000Z',
        location: 'Lakeside Path, Toronto',
        latitude: 43.6532,
        longitude: -79.3832,
        organizer: 'Toronto Cares Foundation',
        organizerId: 'user-1',
        tags: ['#charity', '#5k', '#run', '#community'],
        visibility: 'public',
        ticketTiers: [
            { id: 't22-1', name: 'Runner Registration', price: 30, type: 'paid' },
            { id: 't22-2', name: 'Support Donation', price: 0, type: 'donation' }
        ],
        promoCodes: [
            { code: 'RUNFORFUN', discountPercentage: 5 }
        ]
    },
    {
        id: '23',
        title: 'Indie Film Festival',
        description: 'A showcase of groundbreaking independent films from around the world.',
        richDescription: '<p>The <strong>Indie Film Festival</strong> is back for its 10th year, celebrating the art of storytelling through film. Join us for a week of screenings, filmmaker panels, and networking events. Discover the next generation of cinematic talent.</p>',
        imageUrl: getImage('theater show').imageUrl,
        imageHint: 'theater show',
        videoUrl: undefined,
        category: 'Art',
        date: '2024-10-25T17:00:00.000Z',
        location: 'The Indieplex, Austin',
        latitude: 30.2672,
        longitude: -97.7431,
        organizer: 'Film Enthusiasts Society',
        organizerId: 'user-3',
        tags: ['#film', '#independentfilm', '#festival', '#cinema'],
        visibility: 'public',
        ticketTiers: [
            { id: 't23-1', name: 'Day Pass', price: 40, type: 'paid' },
            { id: 't23-2', name: 'Full Festival Pass', price: 150, type: 'paid' }
        ],
        promoCodes: []
    },
    {
        id: '24',
        title: 'Hack for Good',
        description: 'A 24-hour hackathon to build solutions for social good.',
        richDescription: '<p>Join <strong>Hack for Good</strong>, a 24-hour coding marathon where developers, designers, and innovators come together to build technology solutions for non-profits and social enterprises. Food, drinks, and mentorship provided.</p>',
        imageUrl: getImage('tech conference').imageUrl,
        imageHint: 'tech conference',
        videoUrl: undefined,
        category: 'Technology',
        date: '2024-11-15T18:00:00.000Z',
        location: 'TechHub, Berlin',
        latitude: 52.5200,
        longitude: 13.4050,
        organizer: 'Code for a Cause',
        organizerId: 'user-2',
        tags: ['#hackathon', '#tech', '#socialgood', '#coding'],
        visibility: 'public',
        ticketTiers: [
            { id: 't24-1', name: 'Participant (Free)', price: 0, type: 'reservation' }
        ],
        promoCodes: []
    },
    {
        id: '25',
        title: 'Classical Masters',
        description: 'An evening of breathtaking classical music from Beethoven and Mozart.',
        richDescription: '<p>The City Symphony Orchestra presents <strong>Classical Masters</strong>, an unforgettable evening featuring Beethoven\'s Symphony No. 5 and Mozart\'s Requiem. A powerful performance that will move both new and seasoned classical music lovers.</p>',
        imageUrl: getImage('music concert').imageUrl,
        imageHint: 'music concert',
        videoUrl: undefined,
        category: 'Music',
        date: '2024-12-05T19:30:00.000Z',
        location: 'Symphony Hall, Vienna',
        latitude: 48.2082,
        longitude: 16.3738,
        organizer: 'City Symphony Orchestra',
        organizerId: 'user-1',
        tags: ['#classical', '#orchestra', '#symphony', '#music'],
        visibility: 'public',
        ticketTiers: [
            { id: 't25-1', name: 'Standard Seat', price: 60, type: 'paid' },
            { id: 't25-2', name: 'Premium Seat', price: 120, type: 'paid' }
        ],
        promoCodes: []
    },
    {
        id: '26',
        title: 'Silent Meditation Retreat',
        description: 'A 3-day silent retreat to deepen your meditation practice.',
        richDescription: '<p>Embark on a journey of inner peace at our <strong>Silent Meditation Retreat</strong>. This 3-day retreat provides a supportive environment for you to deepen your practice through guided sessions, walking meditation, and periods of silence. All meals are vegetarian.</p>',
        imageUrl: getImage('yoga workshop').imageUrl,
        imageHint: 'yoga workshop',
        videoUrl: undefined,
        category: 'Wellness',
        date: '2024-10-18T15:00:00.000Z',
        location: 'Mountain Monastery, Kyoto',
        latitude: 35.0116,
        longitude: 135.7681,
        organizer: 'Mindful Path',
        organizerId: 'user-3',
        tags: ['#meditation', '#silentretreat', '#mindfulness', '#wellness'],
        visibility: 'public',
        ticketTiers: [
            { id: 't26-1', name: 'Retreat Package', price: 400, type: 'paid' }
        ],
        promoCodes: []
    },
    {
        id: '27',
        title: 'Artisan Cheese & Wine Pairing',
        description: 'An evening of exquisite cheese and wine pairings.',
        richDescription: '<p>Join us for an <strong>Artisan Cheese & Wine Pairing</strong> night. A sommelier and a fromager will guide you through a selection of six unique cheeses, each paired with a carefully selected wine to enhance its flavor. A delight for the senses.</p>',
        imageUrl: getImage('food festival').imageUrl,
        imageHint: 'food festival',
        videoUrl: undefined,
        category: 'Food',
        date: '2024-11-22T19:00:00.000Z',
        location: 'The Vintner\'s Room, Napa Valley',
        latitude: 38.5025,
        longitude: -122.2654,
        organizer: 'Napa Culinary Experiences',
        organizerId: 'user-1',
        tags: ['#wine', '#cheese', '#tasting', '#foodie'],
        visibility: 'public',
        ticketTiers: [
            { id: 't27-1', name: 'Tasting Experience', price: 95, type: 'paid' }
        ],
        promoCodes: []
    },
    // Adding more events to reach ~15-20 per category
    // Music
    {
        id: '28',
        title: 'Acoustic Cafe Nights',
        description: 'Intimate performances by local singer-songwriters.',
        richDescription: '<p>Enjoy a cozy evening at our <strong>Acoustic Cafe Nights</strong>, where local talent shines. Grab a coffee and relax to the heartfelt tunes of our city\'s best singer-songwriters.</p>',
        imageUrl: getImage('music concert').imageUrl,
        imageHint: 'music concert',
        category: 'Music',
        date: '2024-10-04T19:00:00.000Z',
        location: 'The Daily Grind, Portland',
        latitude: 45.5051,
        longitude: -122.6750,
        organizer: 'Local Vibes',
        organizerId: 'user-1',
        tags: ['#acoustic', '#livemusic', '#singersongwriter'],
        visibility: 'public',
        ticketTiers: [{ id: 't28-1', name: 'Entry', price: 10, type: 'paid' }],
        promoCodes: [],
    },
    {
        id: '29',
        title: 'EDM Explosion',
        description: 'A high-energy electronic dance music festival.',
        richDescription: '<p>Get ready for <strong>EDM Explosion</strong>, a festival with world-renowned DJs, stunning visuals, and a massive sound system. Dance the night away under the electric sky.</p>',
        imageUrl: getImage('music concert').imageUrl,
        imageHint: 'music concert',
        category: 'Music',
        date: '2024-11-09T20:00:00.000Z',
        location: 'The Mega Dome, Las Vegas',
        latitude: 36.1699,
        longitude: -115.1398,
        organizer: 'RaveNation',
        organizerId: 'user-4',
        tags: ['#edm', '#festival', '#dj', '#dance'],
        visibility: 'public',
        ticketTiers: [{ id: 't29-1', name: 'General Admission', price: 95, type: 'paid' }],
        promoCodes: [],
    },
    {
        id: '30',
        title: 'Learn to DJ Workshop',
        description: 'A hands-on workshop for aspiring DJs.',
        richDescription: '<p>Ever wanted to be a DJ? Our <strong>Learn to DJ Workshop</strong> covers the basics of beatmatching, mixing, and scratching. No experience necessary, all equipment provided.</p>',
        imageUrl: getImage('music concert').imageUrl,
        imageHint: 'music concert',
        category: 'Music',
        date: '2024-10-19T13:00:00.000Z',
        location: 'Beat Lab, Brooklyn',
        latitude: 40.6782,
        longitude: -73.9442,
        organizer: 'Scratch Academy',
        organizerId: 'user-1',
        tags: ['#dj', '#workshop', '#musicproduction'],
        visibility: 'public',
        ticketTiers: [{ id: 't30-1', name: 'Workshop Fee', price: 75, type: 'paid' }],
        promoCodes: [],
    },
    // Technology
    {
        id: '31',
        title: 'Intro to Python Coding Bootcamp',
        description: 'A beginner-friendly bootcamp to learn Python.',
        richDescription: '<p>Start your coding journey with our <strong>Intro to Python Bootcamp</strong>. This intensive weekend course will take you from zero to building your first application. Perfect for absolute beginners.</p>',
        imageUrl: getImage('tech conference').imageUrl,
        imageHint: 'tech conference',
        category: 'Technology',
        date: '2024-10-26T10:00:00.000Z',
        location: 'CodeHub, San Francisco',
        latitude: 37.7749,
        longitude: -122.4194,
        organizer: 'The Coding School',
        organizerId: 'user-2',
        tags: ['#python', '#coding', '#bootcamp', '#programming'],
        visibility: 'public',
        ticketTiers: [{ id: 't31-1', name: 'Enrollment', price: 300, type: 'paid' }],
        promoCodes: [],
    },
    {
        id: '32',
        title: 'Cybersecurity Summit',
        description: 'Protect your digital world. A summit on the latest in cybersecurity.',
        richDescription: '<p>Join security experts at the <strong>Cybersecurity Summit</strong> to discuss the latest threats, trends, and technologies in the field. Learn how to protect your business and personal data.</p>',
        imageUrl: getImage('tech conference').imageUrl,
        imageHint: 'tech conference',
        category: 'Technology',
        date: '2024-11-20T09:00:00.000Z',
        location: 'The Secure Zone, Washington D.C.',
        latitude: 38.9072,
        longitude: -77.0369,
        organizer: 'InfoSec Professionals',
        organizerId: 'user-2',
        tags: ['#cybersecurity', '#infosec', '#privacy', '#conference'],
        visibility: 'public',
        ticketTiers: [{ id: 't32-1', name: 'Professional Pass', price: 250, type: 'paid' }],
        promoCodes: [],
    },
    {
        id: '33',
        title: 'Gaming Expo 2024',
        description: 'The biggest gaming event of the year. Play upcoming titles and meet developers.',
        richDescription: '<p><strong>Gaming Expo 2024</strong> is where the gaming world comes together. Get hands-on with unreleased games, watch exclusive demos, and meet the creators behind your favorite titles.</p>',
        imageUrl: getImage('tech conference').imageUrl,
        imageHint: 'tech conference',
        category: 'Technology',
        date: '2024-08-30T10:00:00.000Z',
        location: 'Convention Center, Los Angeles',
        latitude: 34.0522,
        longitude: -118.2437,
        organizer: 'GameCon',
        organizerId: 'user-4',
        tags: ['#gaming', '#videogames', '#expo', '#e3'],
        visibility: 'public',
        ticketTiers: [{ id: 't33-1', name: 'Day Pass', price: 60, type: 'paid' }],
        promoCodes: [],
    },
    // Art
    {
        id: '34',
        title: 'Watercolor Painting Workshop',
        description: 'Learn the basics of watercolor painting in a beautiful garden setting.',
        richDescription: '<p>Unleash your creativity at our <strong>Watercolor Painting Workshop</strong>. Surrounded by the beauty of a botanical garden, you\'ll learn techniques for washes, blending, and detail work. All materials included.</p>',
        imageUrl: getImage('art exhibition').imageUrl,
        imageHint: 'art exhibition',
        category: 'Art',
        date: '2024-09-14T10:00:00.000Z',
        location: 'Botanical Gardens, Sydney',
        latitude: -33.8688,
        longitude: 151.2093,
        organizer: 'Creative Canvas',
        organizerId: 'user-3',
        tags: ['#watercolor', '#painting', '#workshop', '#artclass'],
        visibility: 'public',
        ticketTiers: [{ id: 't34-1', name: 'Workshop Fee', price: 85, type: 'paid' }],
        promoCodes: [],
    },
    {
        id: '35',
        title: 'Photography Walk: City Architecture',
        description: 'Capture the architectural beauty of the city with a professional photographer.',
        richDescription: '<p>Join our <strong>Photography Walk</strong> and explore the city\'s most iconic buildings. A professional photographer will guide you on composition, lighting, and camera settings to capture stunning architectural shots.</p>',
        imageUrl: getImage('art exhibition').imageUrl,
        imageHint: 'art exhibition',
        category: 'Art',
        date: '2024-10-12T09:00:00.000Z',
        location: 'Downtown, Chicago',
        latitude: 41.8781,
        longitude: -87.6298,
        organizer: 'Shutterbugs United',
        organizerId: 'user-3',
        tags: ['#photography', '#photowalk', '#architecture', '#workshop'],
        visibility: 'public',
        ticketTiers: [{ id: 't35-1', name: 'Tour Ticket', price: 40, type: 'paid' }],
        promoCodes: [],
    },
    // Food
    {
        id: '36',
        title: 'Vegan Cooking Masterclass',
        description: 'Learn to cook delicious and creative plant-based meals.',
        richDescription: '<p>Discover the world of vegan cuisine in our <strong>Vegan Cooking Masterclass</strong>. Learn how to create flavorful and satisfying dishes using only plant-based ingredients. You\'ll prepare a 3-course meal and enjoy it at the end.</p>',
        imageUrl: getImage('food festival').imageUrl,
        imageHint: 'food festival',
        category: 'Food',
        date: '2024-11-16T18:00:00.000Z',
        location: 'The Green Kitchen, Austin',
        latitude: 30.2672,
        longitude: -97.7431,
        organizer: 'Plant-Powered Plates',
        organizerId: 'user-1',
        tags: ['#vegan', '#cookingclass', '#plantbased', '#foodie'],
        visibility: 'public',
        ticketTiers: [{ id: 't36-1', name: 'Class Fee', price: 110, type: 'paid' }],
        promoCodes: [],
    },
    {
        id: '37',
        title: 'Oktoberfest Celebration',
        description: 'Enjoy traditional German beer, food, and music.',
        richDescription: '<p>Prost! Come celebrate <strong>Oktoberfest</strong> with us. We\'ll have a wide selection of German beers, traditional food like bratwurst and pretzels, and live oompah music. Lederhosen and dirndls encouraged!</p>',
        imageUrl: getImage('food festival').imageUrl,
        imageHint: 'food festival',
        category: 'Food',
        date: '2024-10-05T12:00:00.000Z',
        location: 'Biergarten, Munich',
        latitude: 48.1351,
        longitude: 11.5820,
        organizer: 'Das Beer Fest',
        organizerId: 'user-4',
        tags: ['#oktoberfest', '#beer', '#germanfood', '#festival'],
        visibility: 'public',
        ticketTiers: [{ id: 't37-1', name: 'Entry + 1 Beer', price: 20, type: 'paid' }],
        promoCodes: [],
    },
    // Business
    {
        id: '38',
        title: 'Digital Marketing Mastery',
        description: 'A 2-day conference on the latest trends in digital marketing.',
        richDescription: '<p>Stay ahead of the curve with our <strong>Digital Marketing Mastery</strong> conference. Learn from experts about SEO, social media marketing, content strategy, and AI-powered marketing tools.</p>',
        imageUrl: getImage('business seminar').imageUrl,
        imageHint: 'business seminar',
        category: 'Business',
        date: '2024-09-19T09:00:00.000Z',
        location: 'Online Conference',
        latitude: 0,
        longitude: 0,
        organizer: 'Marketing Mavens',
        organizerId: 'user-2',
        tags: ['#digitalmarketing', '#seo', '#socialmedia', '#business'],
        visibility: 'public',
        ticketTiers: [{ id: 't38-1', name: 'Virtual Ticket', price: 199, type: 'paid' }],
        promoCodes: [],
    },
    {
        id: '39',
        title: 'Networking for Introverts',
        description: 'A workshop designed to make networking less intimidating and more effective.',
        richDescription: '<p>Hate networking? Our <strong>Networking for Introverts</strong> workshop will teach you practical strategies to build meaningful professional connections without the awkwardness. A safe and supportive environment.</p>',
        imageUrl: getImage('business seminar').imageUrl,
        imageHint: 'business seminar',
        category: 'Business',
        date: '2024-10-22T18:30:00.000Z',
        location: 'The Quiet Corner, Seattle',
        latitude: 47.6062,
        longitude: -122.3321,
        organizer: 'Connect & Thrive',
        organizerId: 'user-2',
        tags: ['#networking', '#career', '#workshop', '#introvert'],
        visibility: 'public',
        ticketTiers: [{ id: 't39-1', name: 'Workshop Fee', price: 50, type: 'paid' }],
        promoCodes: [],
    },
    // Wellness
    {
        id: '40',
        title: 'Holistic Nutrition Seminar',
        description: 'Learn how to nourish your body and mind with whole foods.',
        richDescription: '<p>Join our <strong>Holistic Nutrition Seminar</strong> to learn about the power of food as medicine. A certified nutritionist will discuss topics like gut health, anti-inflammatory diets, and mindful eating.</p>',
        imageUrl: getImage('yoga workshop').imageUrl,
        imageHint: 'yoga workshop',
        category: 'Wellness',
        date: '2024-11-02T11:00:00.000Z',
        location: 'Wellness Center, Boulder',
        latitude: 40.0150,
        longitude: -105.2705,
        organizer: 'Nourish Yourself',
        organizerId: 'user-3',
        tags: ['#nutrition', '#health', '#wellness', '#seminar'],
        visibility: 'public',
        ticketTiers: [{ id: 't40-1', name: 'Admission', price: 35, type: 'paid' }],
        promoCodes: [],
    },
    {
        id: '41',
        title: 'Guided Nature Hike',
        description: 'A refreshing hike through the national park with a knowledgeable guide.',
        richDescription: '<p>Reconnect with nature on our <strong>Guided Nature Hike</strong>. We\'ll explore a beautiful trail while our guide points out local flora and fauna and shares stories about the area\'s history. A great way to de-stress and get some exercise.</p>',
        imageUrl: getImage('yoga workshop').imageUrl,
        imageHint: 'yoga workshop',
        category: 'Wellness',
        date: '2024-09-08T10:00:00.000Z',
        location: 'Yosemite National Park, CA',
        latitude: 37.8651,
        longitude: -119.5383,
        organizer: 'Outdoor Adventures',
        organizerId: 'user-3',
        tags: ['#hiking', '#nature', '#outdoors', '#wellness'],
        visibility: 'public',
        ticketTiers: [{ id: 't41-1', name: 'Hike Ticket', price: 25, type: 'paid' }],
        promoCodes: [],
    },
    // Sports
    {
        id: '42',
        title: 'City Marathon 2024',
        description: 'The annual city marathon. Challenge yourself and run through the iconic streets.',
        richDescription: '<p>The <strong>City Marathon 2024</strong> is the city\'s premier running event. Whether you\'re a seasoned marathoner or a first-timer, this is an unforgettable experience. The course takes you past all the major landmarks.</p>',
        imageUrl: getImage('sports game').imageUrl,
        imageHint: 'sports game',
        category: 'Sports',
        date: '2024-10-27T07:00:00.000Z',
        location: 'Start Line, Central Plaza',
        latitude: 40.7128,
        longitude: -74.0060,
        organizer: 'City Runners Club',
        organizerId: 'user-4',
        tags: ['#marathon', '#running', '#race', '#fitness'],
        visibility: 'public',
        ticketTiers: [{ id: 't42-1', name: 'Runner Registration', price: 120, type: 'paid' }],
        promoCodes: [],
    },
    {
        id: '43',
        title: 'Rock Climbing Basics',
        description: 'Learn the fundamentals of indoor rock climbing.',
        richDescription: '<p>New to climbing? Our <strong>Rock Climbing Basics</strong> class will teach you everything you need to know to get started, including safety procedures, basic knots, and climbing techniques. All equipment included.</p>',
        imageUrl: getImage('sports game').imageUrl,
        imageHint: 'sports game',
        category: 'Sports',
        date: '2024-09-15T18:00:00.000Z',
        location: 'The Climbing Gym, Denver',
        latitude: 39.7392,
        longitude: -104.9903,
        organizer: 'Ascend Climbing',
        organizerId: 'user-4',
        tags: ['#rockclimbing', '#bouldering', '#fitness', '#workshop'],
        visibility: 'public',
        ticketTiers: [{ id: 't43-1', name: 'Intro Class', price: 55, type: 'paid' }],
        promoCodes: [],
    },
    // Community
    {
        id: '44',
        title: 'Board Game Cafe Meetup',
        description: 'Meet new people and play some classic and modern board games.',
        richDescription: '<p>Join our friendly <strong>Board Game Cafe Meetup</strong>. We have a huge library of games to choose from, and our hosts are happy to teach new players. A great, low-pressure way to socialize.</p>',
        imageUrl: getImage('community meetup').imageUrl,
        imageHint: 'community meetup',
        category: 'Community',
        date: '2024-11-13T19:00:00.000Z',
        location: 'The Meeple\'s Corner, Toronto',
        latitude: 43.6532,
        longitude: -79.3832,
        organizer: 'Toronto Board Gamers',
        organizerId: 'user-1',
        tags: ['#boardgames', '#meetup', '#social', '#games'],
        visibility: 'public',
        ticketTiers: [{ id: 't44-1', name: 'Cover Charge', price: 10, type: 'paid' }],
        promoCodes: [],
    },
    {
        id: '45',
        title: 'Annual Charity Gala',
        description: 'A formal evening of dining and dancing to support a good cause.',
        richDescription: '<p>You are cordially invited to our <strong>Annual Charity Gala</strong>. This elegant black-tie event features a gourmet dinner, live auction, and dancing, with all proceeds benefiting the Children\'s Hospital Foundation.</p>',
        imageUrl: getImage('community meetup').imageUrl,
        imageHint: 'community meetup',
        category: 'Community',
        date: '2024-12-07T18:00:00.000Z',
        location: 'The Grand Ballroom, Beverly Hills',
        latitude: 34.0736,
        longitude: -118.4004,
        organizer: 'Children\'s Hospital Foundation',
        organizerId: 'user-4',
        tags: ['#charity', '#gala', '#fundraiser', '#formal'],
        visibility: 'public',
        ticketTiers: [{ id: 't45-1', name: 'Individual Ticket', price: 500, type: 'paid' }],
        promoCodes: [],
    }
];

export const categories = [
    'Music',
    'Technology',
    'Art',
    'Food',
    'Business',
    'Wellness',
    'Sports',
    'Community'
];
